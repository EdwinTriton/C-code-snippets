...
Vektor3d() {for (int i=0; i<3; i++) kordinate[i]=0;}
Vektor3d(const Vektor3d &v) {for (int i=0; i<3; i++) kordinate[i]=v.kordinate[i];brojispisa=0;}
...
void Ispisi() const { std::cout << "{" << kordinate[0] << "," << kordinate[1] << "," << kordinate[2] << "}"; brojispisa++;}
